...
type ResumeData = { label:string; target?:string; template?:'classic'|'sidebar'|'compact'; theme?:'blue'|'emerald'|'violet'|'rose'|'slate'; basics:{name:string;title?:string;email?:string;phone?:string;location?:string;website?:string;github?:string;summary?:string}; education?:{school:string;degree?:string;major?:string;period?:string;gpa?:string;detail?:string}[]; work?:{company:string;position?:string;period?:string;location?:string;highlights?:string[]}[]; projects?:{name:string;role?:string;period?:string;tech?:string[];highlights?:string[];link?:string}[]; skills?:{category?:string;items:string[]}[]; awards?:{title:string;issuer?:string;date?:string}[] };
规则：保留候选人真实信息，不要编造经历或数字；针对 JD 调整措辞与条目顺序，突出匹配点；要点简洁有力，可用 **粗体** 强调关键成果/数字；语言与 JD 保持一致（默认中文）。`;
